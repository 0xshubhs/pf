...
const StatusBadge = ({ status }: { status: PrStatus }) =>
  status === 'merged' ? (
    ...
    <span className="inline-flex items-center gap-1 rounded-full border border-gray-400/30 bg-gray-400/10 px-2 py-0.5 text-[11px] font-medium text-gray-600 dark:text-gray-400 shrink-0">
            ...
            {c.prs.map((pr) => (
              <li key={pr.url}>
                ...
                >
                  ...
                  <StatusBadge status={liveStatuses[pr.url] ?? pr.status} />
                </a>
              </li>
            ))}
          </ul>
